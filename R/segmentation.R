...
segments <- function(RCA,
    ...
    threshold <- switch(thr_type, 'mean' = min_segment_mean, 'median' = min_segment_median)
    colname <- switch(thr_type, 'mean' = 'seg.mean', 'median' = 'seg.median')
    
    param <- validate_segmentation_params(alpha, min_width, undo_SD)
    
    target_pairs_df <- validate_target_pairs(
        target_pairs,
        RCA$meta$don_chr_ids,
        RCA$meta$rec_chr_ids
    )
    ...
    prepared_data <- target_pairs_df |>
        dplyr::mutate(
            ...
            segments = purrr::pmap(
                list(.data$data, .data$chr_id.don, .data$chr_id.rec),
                \(x, don, rec) {
                    if (nrow(x) < 2L) {
                        warning(
                            sprintf(
                                paste0(
                                    "Skipping donor %s / recipient %s: ",
                                    "need at least 2 ideal windows for CBS, got %d"
                                ),
                                don,
                                rec,
                                nrow(x)
...
subset.ISA <- function(x,
    ...
    if (!is.null(threshold_type)) {
        stopifnot("Threshold types must be 'mean' or 'median'" =
                      !is.na(match(
        ...
        if (!identical(new_metrics, x$meta$metrics)) {
            if (identical(threshold_type, 'mean') && is.null(min_segment_mean)) {
                stop(
                    "When switching threshold_type to 'mean', supply min_segment_mean",
                    call. = FALSE
                )
            }
            if (identical(threshold_type, 'median') && is.null(min_segment_median)) {
                stop(
    ...
    if (!is.null(min_width)) {
        ...
        x$segments <- dplyr::filter(x$segments,
...
plot.ISA <- function(x,
    ...
    if (is.null(x$out) || nrow(x$out) == 0L) {
        stop(
            "No pairwise windows available to plot",
            call. = FALSE
        )
    }
...
summary.ISA <- function(object,
    ...
    intro_data <- annotate_segment_coords(
        ...
        dplyr::mutate(length.don = .data$stop.don - .data$start.don,
                      .after = 'stop.don') |>
        dplyr::mutate(length.rec = .data$stop.rec - .data$start.rec,
                      .after = 'stop.rec')
...
print.summary.ISA <- function(x, ...) {
    ...
    if (x$n_introgressions == 0L) {
        ...
        cat('Putative introgressions found:',
            x$n_introgressions,
            '\n')
        cat('Coordinates and length reported in',
            x$unit,
            'scale',
            '\n\n')
